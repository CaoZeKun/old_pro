...
class the_model(nn.Module):
    def __init__(self,input_size_lnc,input_size_A,input_size_lncmi,input_size_dis,input_size_midis,output_size,batch_size):
        ...
        self.fully_connected = nn.Sequential(nn.Linear(109440, 2), nn.BatchNorm1d(2))
...
def test4(k):
    ...
    for i in range(k):
        ...
        # X_Y_P_R_test[0].extend(AUC_all_column)
        # X_Y_P_R_test[1].extend(PR_all_column)
    ...
    print(d)
    # np.savetxt('CNNLDA_PR_test1.txt', X_Y_P_R_test)
    ...
    # CNNLDA_PR.append(R)
    # CNNLDA_PR.append(P)
    # np.savetxt('CNNLDA_PR_1.txt',CNNLDA_PR)
    # save_to_file2('cnnlncRNA.txt', d)
...
def predict_lda():
    ...
    for i in range(length_test):
        all_column_test_data[test_data[i].index_y].append(test_data[i])  # ------pay attention append or extend
    # according to different column
    result_69, result_113, result_171 = predict_column(model,columns,all_column_test_data,A, Sm, Sd,Lm, Md)
    np.savetxt('results_predict_69.txt',result_69,fmt='%d')
    np.savetxt('results_predict_113.txt', result_113,fmt='%d')
    np.savetxt('results_predict_171.txt', result_171,fmt='%d')
