...
def predict_lda(k):
    ...
    all_k_Sd_associated, all_k_count_positive, all_k_test_data, num, A_length = data_prepare(A,k)
    epoch1 = 80
    _LR = 0.0001
    batchsize = 50
    # length of row
    ...
    for i in range(k):
        ...
        train_loader = load_data2(all_k_Sd_associated[i], A[i],Sm,Sd,batchsize,Lm,Md)
        model = the_model(input_size_lnc,input_size_A,input_size_lncmi,input_size_dis,input_size_midis,output_size,batchsize).cuda()
        ...
        for i in range(length_test):
            all_column_test_data[test_data[i].index_y].append(test_data[i])  # ------pay attention append or extend
        # according to different column
        Y_TPR_all_columns, X_FPR_all_columns, Y_P_all_columns, X_R_all_columns, areaROC, areaPR = predict_column(model,all_column_test_data,A,Sm,Sd,Lm,Md)
