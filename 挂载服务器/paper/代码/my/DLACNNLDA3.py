...
def data_prepare(matrix_A):
    ...
    save_count_zero = copy.deepcopy(save_all_count_zero_not_changed)  # temp save zero
    train_samples = []  # save  samples which conclude positive and negative ---save train data

    column_length = matrix_A.shape[1]  # the length of column    diseases 326
    row_length = matrix_A.shape[0]  # the length of row        miRNA 429
    ...
    for j in range(column_length):
        for i in range(row_length):
            if (matrix_A[i][j] == 1):  # use new associated_A get positive
                count_positive += 1
                # because save_all_count_A come from matrix A, so location i and j not change ,so could use i ,j
                data = data_input(1, i, j)
                k_Sd_associated.append(data)  # save a certain disease  with certain miRNA  label is 1
    ...
    # train sets
    train_samples.extend(k_Sd_associated[:])
    train_samples.extend(k_Sd_negative[:])  # save in a raw
    ...
    random.shuffle(save_all_count_zero)
    test_save = []
    for i in range(length):
        test_save.append(
            data_input(save_all_count_zero[i].value, save_all_count_zero[i].value_x, save_all_count_zero[i].value_y))
    ...
    return train_samples,  test_save
...
def predict_column(cnn,columns,column_test_data,Sd_associated,Sm,Sd,Lm,Md):
    ...
    for i in range(columns):
        ...
        if(i== 69):
            ...
            # b = sorted(results_temp1, key=lambda x:x[5], reverse=True)
            for j in range(length_test):
                results_temp1.extend([temp_test_data[j].index_x])

                # temp_1.extend([temp_test_data[j].index_x])
                # temp_2.extend([temp_test_data[j].probability])
            # results_temp1.append(temp_1)
            # results_temp1.append(temp_2)
            # print(results_temp1)
        ...
        if (i == 113):
            ...
            # b = sorted(results_temp1, key=lambda x:x[5], reverse=True)
            for j in range(length_test):
                results_temp2.extend([temp_test_data[j].index_x])

                # temp_1.extend([temp_test_data[j].index_x])
                # temp_2.extend([temp_test_data[j].probability])
            # results_temp2.append(temp_1)
            # results_temp2.append(temp_2)
            # print(results_temp2)
        ...
        if (i == 383):
            ...
            # b = sorted(results_temp1, key=lambda x:x[5], reverse=True)
            for j in range(length_test):
                results_temp3.extend([temp_test_data[j].index_x])

                # temp_1.extend([temp_test_data[j].index_x])
                # temp_2.extend([temp_test_data[j].probability])
            # results_temp3.append(temp_1)
            # results_temp3.append(temp_2)
            # print(results_temp3)



    return results_temp1, results_temp2, results_temp3


def save_to_file3(file_name, contents1,contents2,contents3,contents4):
    with open(file_name, 'a') as f:
        f.write(contents1 + '\t'+contents2 +'\t'+contents3 +'\t'+contents4 + '\n')
def predict_column1(cnn,columns,column_test_data,Sd_associated,Sm,Sd,Lm,Md):


    dis_name = pd.read_table("../data_create/disease_name.txt",header=None,encoding='gbk')

    lnc_name = pd.read_table("../data_create/yuguoxian_lncRNA_name.txt",header=None,encoding='gbk')
    predict_results = []
    # according to column
    results_temp1 = []
    ...
    for i in range(columns):
        ...
        for j in range(length_test):
            if(j < 50):
                # results_temp1.extend([dis_name.iloc[i,2],lnc_name.iloc[temp_test_data[j].index_x]])
                save_to_file3('predict_dis_lnc.txt', dis_name.iloc[i,2],
                              lnc_name.iloc[temp_test_data[j].index_x,0],str(j),str(temp_test_data[j].probability))
...
def train5_wd(model,EPOCH,train_loader,test_loader,LR,wd):
...
def train3(model,EPOCH,train_loader,test_loader,LR):
    ...
    optimizer = torch.optim.Adagrad(model.parameters(), lr=LR, weight_decay=1e-8)
    #scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.1)
    loss_func = nn.CrossEntropyLoss()
    #exp_lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)
...
def predict_top(cnn,columns,column_test_data,Sd_associated,Sm,Sd,Lm,Md):
    all_predict_test = []
...
def draw_roc_column(cnn,columns,column_test_data,Sd_associated,Sm,Sd,Lm,Md):
    ...
    return Y_TPR_all_columns,X_FPR_all_columns,Y_P_all_columns,X_R_all_columns,areaROC,areaPR,Rcall_all
...
def get_current_AUC_ROC(cnn,test_data,columns,Sd_associated,Sm,Sd,Lm,Md,time):
    ...
    Y_TPR_all_columns, X_FPR_all_columns, Y_P_all_columns, X_R_all_columns, areaROC, areaPR,Rcall_all = draw_roc_column(cnn,columns, all_column_test_data,Sd_associated,Sm,Sd,Lm,Md)
    ...
    #plt.show()
    return x_FPR,y_TPR,x_R,y_P,areaROC, areaPR,Rcall
...
def test4(k):
    ...
    for i in range(k):
        ...
        x_FPR, y_TPR, x_R, y_P, areaROC, areaPR,Rcall = get_current_AUC_ROC(model, test_data, len(A[0]),save_all_count_A[i],Sm,Sd,Lm,Md,i)
...
def test5(k):
    ...
    _LR = 0.0001
    batchsize = 50
    which_LR = {'0.1': 0.1,'0.05': 0.05,'0.01': 0.01,'0.005': 0.005, '0.001': 0.001, '0.0005': 0.0005, '0.0001': 0.0001}
    which_wd = {'1000': 1000, '10': 10, '1': 1, '0.1': 0.1, '0.001': 0.001, '0.0001': 0.0001,'1e-6': 1e-6,'1e-8': 1e-8}
    # length of row
    input_size_lnc = len(A)  # 240
    input_size_A = len(A[0])  # 405
    input_size_lncmi = len(Lm[0])  # 495
    ...
    for wd in which_wd.values():
        ...
        for i in range(k):
            ...
            train_loader = load_data2(all_k_Sd_associated[i], save_all_count_A[i], Sm, Sd, batchsize, Lm, Md)
            model = the_model(input_size_lnc, input_size_A, input_size_lncmi, input_size_dis, input_size_midis,
                              output_size, batchsize).cuda()

            model.train()
            train5_wd(model, epoch1, train_loader, dev_loader, _LR,wd)
            # train3(model, epoch2, dev_loader, train_loader, _LR)
            # train4(cnn, epoch2, train_loader, dev_loader, _LR)
            model.eval()
            test_data = all_k_test_data[i]
            ...
            # # # according to different column
            # areaROC0, areaPR0 = draw_roc_column1(model, rows,all_column_test_data,save_all_count_A[i],Sm,Sd,Lm,Md)
            x_FPR, y_TPR, x_R, y_P, areaROC, areaPR, Rcall = get_current_AUC_ROC(model, test_data, len(A[0]),
                                                                                 save_all_count_A[i], Sm, Sd, Lm, Md, i)
            Rcall_all.append(Rcall)
            X_all_k_FPR.append(x_FPR)
            Y_all_k_TPR.append(y_TPR)
            X_all_k_R.append(x_R)
        ...
        print(d)
        # get_all_k_Curve(X_all_k_FPR, Y_all_k_TPR, X_all_k_R, Y_all_k_P, k)
        save_to_file2('CNNLDA_params.txt', d)
...
def predict_lda():
    ...
    all_associated, test_data = data_prepare(A)
    epoch1 = 80
    _LR = 0.0001
    # print(1)
    batchsize = 50
    ...
    Sm = get_all_sim1(A, Sd)

    train_loader = load_data2(all_associated, A, Sm, Sd, batchsize, Lm, Md)
    model = the_model(input_size_lnc, input_size_A, input_size_lncmi, input_size_dis, input_size_midis, output_size,
                      batchsize).cuda()

    model.train()
    train_top(model, epoch1, train_loader, _LR)
    model.eval()
    ...
    for i in range(length_test):
        all_column_test_data[test_data[i].index_y].append(test_data[i])  # ------pay attention append or extend
    # according to different column
    predict_column1(model, columns, all_column_test_data, A, Sm, Sd, Lm, Md)
    # result_69, result_113, result_383 = predict_column(model,columns,all_column_test_data,A, Sm, Sd,Lm, Md)
    # np.savetxt('results_predict_69.txt',result_69,fmt='%d')
    # np.savetxt('results_predict_113.txt', result_113,fmt='%d')
    # np.savetxt('results_predict_383.txt', result_383,fmt='%d')
