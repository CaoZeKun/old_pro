...
def data_prepare(matrix_A,k):
    _, _, save_all_count_zero_not_changed, _, num, A_length  = crossvalidation(matrix_A,k)
    all_k_Sd_associated = []  # save all k * 1 * 326   k * disease * diseases   if(A(i,j) =1)
    all_k_count_positive = []  # save all  k positive length  (every time number of positive samples)
    all_k_development = []  # save all k development sets
    all_k_test_data = []  # save all k test data
    for x in range(k):
        ...
        k_Sd_samples = []  # save  samples which conclude positive and negative ---save train data
        k_Sd_samples_development = []  # save development data
        column_length = len(matrix_A[x][0])  # the length of column    diseases 326
        row_length = len(matrix_A[x])  # the length of row        miRNA 429
        ...
        for j in range(column_length):
            for i in range(row_length):
                if (matrix_A[x][i][j] == 1):  # use new associated_A get positive
                    count_positive += 1
                    # because save_all_count_A come from matrix A, so location i and j not change ,so could use i ,j
                    data = data_input(1, i, j)
                    k_Sd_associated.append(data)  # save a certain disease  with certain miRNA  label is 1
    ...
    return all_k_Sd_associated, all_k_count_positive, all_k_test_data,  num, A_length

def predict_column(cnn,columns,column_test_data,Sd_associated,Sm,Sd,Lm,Md):
    areaROC = 0.0
    areaPR = 0
...
class Beta_score2(nn.Module):
    ...
    def forward(self, result_ls,result_A,result_lm,result_AT,result_ds,result_dm):
        ...
        temp_nodes = torch.tanh(temp_nodes)  # 50, 6, 100
...
class the_model(nn.Module):
    def __init__(self,input_size_lnc,input_size_A,input_size_lncmi,input_size_dis,input_size_midis,output_size,batch_size):
        ...
        self.fully_connected = nn.Sequential(nn.Linear(36480, 2), nn.BatchNorm1d(2))
    ...
    def forward(self, x):
        ...
        # z_i = z_i.view(x.size()[0], -1)
        # print(z_i.size())
        # f_c_att = self.cnn_att(z_i).view(x.size()[0], -1)
        f_c_att = self.cnn_att(z_i)
        ...
        # print(f_c_m.size())
        # print(f_c_m.size())
        f_c_con = torch.cat((f_c_att,f_c_m),dim=2)

        f_c = self.cnn_con(f_c_con).view(x.size()[0], -1)
        # print(f_c.size())
...
def train_top(model,EPOCH,train_loader,LR):
...
def draw_roc_column(cnn,columns,column_test_data,Sd_associated,Sm,Sd,Lm,Md):
    ...
    for i in range(columns):
        ...
        area_column_ROC = area_ROC(TPR_temp, FPR_temp)
        area_column_PR = area_PR(P_temp, R_temp)
        AUC_all_column.extend([area_column_ROC])
        PR_all_column.extend([area_column_PR])
    ...
    return Y_TPR_all_columns,X_FPR_all_columns,Y_P_all_columns,X_R_all_columns,areaROC,areaPR,Rcall_all,AUC_all_column,PR_all_column,test_15_dis_AUC,test_15_dis_PR


def draw_roc_column_test(cnn,columns,column_test_data,Sd_associated,Sm,Sd,Lm,Md):
...
def get_current_AUC_ROC(cnn,test_data,columns,Sd_associated,Sm,Sd,Lm,Md,time):
    ...
    Y_TPR_all_columns, X_FPR_all_columns, Y_P_all_columns, X_R_all_columns, areaROC, areaPR, Rcall_all,AUC_all_column,PR_all_column,test_15_dis_AUC,test_15_dis_PR= draw_roc_column(cnn,columns, all_column_test_data,Sd_associated,Sm,Sd,Lm,Md)
    ...
    #plt.show()
    return x_FPR,y_TPR,x_R,y_P,areaROC, areaPR, Rcall,AUC_all_column,PR_all_column,test_15_dis_AUC,test_15_dis_PR
...
def test4(k):
    ...
    a_a_2 = area_all_PR / k
    # print(a_a)
    d = 'area_ROC:{},area_PR:{},LR:{},epoch1:{}'.format(a_a_1,a_a_2,_LR,epoch1)
    print(d)
...
def test6(k):
    ...
    for i in range(k):
        ...
        # # # according to different column
        # areaROC0, areaPR0 = draw_roc_column1(model, rows,all_column_test_data,save_all_count_A[i],Sm,Sd,Lm,Md)
        x_FPR, y_TPR, x_R, y_P, areaROC, areaPR, Rcall,AUC_all_column,PR_all_column,test_15_dis_AUC,test_15_dis_PR = get_current_AUC_ROC(model, test_data, len(A[0]),save_all_count_A[i],Sm,Sd,Lm,Md,i)
        ...
        del model
        # time += 1
    DL_Rcall = np.sum(Rcall_all, axis=0) / k
    a_a_1 = area_all_ROC/k
    a_a_2 = area_all_PR / k
    # print(a_a)
    d = 'area_ROC:{},area_PR:{},LR:{},epoch1:{},epoch2:{}'.format(a_a_1,a_a_2,_LR,epoch1,epoch2)
    ...
    SIMC_FPR, SIMC_TPR,SIMC_R,SIMC_P,SIMC_AUC,SIMC_PR,SIMC_Rcall = SIMC.test2(A, Sd, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    print(SIMC_Rcall)

    LDAP_FPR, LDAP_TPR,LDAP_R,LDAP_P,LDAP_AUC,LDAP_PR,LDAP_Rcall = LDAP.test2(A, save_all_count_A, save_all_count_zero_not_changed,
                                    save_all_count_zero_every_time_changed, num, A_length, sava_association_A)
    print(LDAP_Rcall)

    ANML_FPR, ANML_TPR,ANML_R,ANML_P,ANML_AUC,ANML_PR,ANML_Rcall = ANML.test3(A, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    print(ANML_Rcall)

    MFLDA_FPR, MFLDA_TPR,MFLDA_R,MFLDA_P,MFLDAC_AUC,MFLDA_PR,MFLDA_Rcall = MFLDA.test2(A, save_all_count_A, sava_association_A, save_all_count_zero_not_changed,
                                       save_all_count_zero_every_time_changed, num, A_length)
    print(MFLDA_Rcall)

    ROC_FPR = []
    ROC_TPR = []
    ROC_FPR.append(DL_FPR)
    ROC_FPR.append(SIMC_FPR)
    ROC_FPR.append(ANML_FPR)
    ROC_FPR.append(MFLDA_FPR)
    ROC_FPR.append(LDAP_FPR)
    ROC_TPR.append(DL_TPR)
    ROC_TPR.append(SIMC_TPR)
    ROC_TPR.append(ANML_TPR)
    ROC_TPR.append(MFLDA_TPR)
    ROC_TPR.append(LDAP_TPR)
    P = []
    R = []
    R.append(DL_R)
    R.append(SIMC_R)
    ...
    P.append(MFLDA_P)
    P.append(LDAP_P)
    np.savetxt('ROC_FPR.txt',ROC_FPR)
    np.savetxt('ROC_TPR.txt', ROC_TPR)
    np.savetxt('R.txt', R)
    np.savetxt('P.txt', P)
    ...
    l1 = plt.legend([p1, p2, p3, p4, p5],
                    ["DLACNNLDA("+str(a_a_1)[:6]+")", "SIMCLDA("+str(SIMC_AUC)[:6]+")", "Ping's method("+str(ANML_AUC)[:6]+")", "MFLDA("+str(MFLDAC_AUC)[:6]+")", "LDAP("+str(LDAP_AUC)[:6]+")"],
                    loc='lower right')
    ...
    plt.savefig("ROC.png")
    plt.show()
    # 'salmon' 'lightgreen' 'lightskyblue' 'gold' 'orchid'
    ...
    l1 = plt.legend([p1, p2, p3, p4, p5],
                    ["DLACNNLDA", "SIMCLDA", "Ping's method", "MFLDA", "LDAP"],
                    loc='upper right')
    plt.xlabel("Recall")
    ...
    ax.set_xticks(x)
    plt.bar(x - 0.3, DL_Rcall, width=width, label='DLACNNLDA', color='salmon')
...
def test6_test(k):
    ...
    for i in range(k):
        ...
        # # # according to different column
        # areaROC0, areaPR0 = draw_roc_column1(model, rows,all_column_test_data,save_all_count_A[i],Sm,Sd,Lm,Md)
        x_FPR, y_TPR, x_R, y_P, areaROC, areaPR, Rcall,AUC_all_column,PR_all_column = get_current_AUC_ROC(model, test_data, len(A[0]),save_all_count_A[i],Sm,Sd,Lm,Md,i)
        CNN_AUC_all_column.extend(AUC_all_column)
        CNN_PR_all_column.extend(PR_all_column)
        del model
        # time += 1

    AUC_test.append(CNN_AUC_all_column)
    PR_test.append(CNN_PR_all_column)
    print("CNN")
    # save_to_file2('cnnlncRNA.txt', d)
    ...
    SIMC_FPR, SIMC_TPR,SIMC_R,SIMC_P,SIMC_AUC,SIMC_PR,SIMC_Rcall,SIMC_AUC_all_column, SIMC_PR_all_column = SIMC.test2_test(A, Sd, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    AUC_test.append(SIMC_AUC_all_column)
    PR_test.append(SIMC_PR_all_column)
    print(SIMC_Rcall)

    LDAP_FPR, LDAP_TPR,LDAP_R,LDAP_P,LDAP_AUC,LDAP_PR,LDAP_Rcall,LDAP_AUC_all_column, LDAP_PR_all_column = LDAP.test2_test(A, save_all_count_A, save_all_count_zero_not_changed,
                                    save_all_count_zero_every_time_changed, num, A_length, sava_association_A)
    print(LDAP_Rcall)
    AUC_test.append(LDAP_AUC_all_column)
    PR_test.append(LDAP_PR_all_column)

    ANML_FPR, ANML_TPR,ANML_R,ANML_P,ANML_AUC,ANML_PR,ANML_Rcall,ANML_AUC_all_column, ANML_PR_all_column = ANML.test3_test(A, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    print(ANML_Rcall)
    AUC_test.append(ANML_AUC_all_column)
    PR_test.append(ANML_PR_all_column)

    MFLDA_FPR, MFLDA_TPR,MFLDA_R,MFLDA_P,MFLDAC_AUC,MFLDA_PR,MFLDA_Rcall,MFLDA_AUC_all_column, MFLDA_PR_all_column = MFLDA.test2_test(A, save_all_count_A, sava_association_A, save_all_count_zero_not_changed,
                                       save_all_count_zero_every_time_changed, num, A_length)
    print(MFLDA_Rcall)
    AUC_test.append(MFLDA_AUC_all_column)
    PR_test.append(MFLDA_PR_all_column)

    np.savetxt('AUC_Ttest.txt',AUC_test)
    np.savetxt('PR_Ttest.txt', PR_test)
...
def test7_test(k):
    ...
    for i in range(k):
        ...
        for i in range(length_test):
            all_column_test_data[test_data[i].index_y].append(test_data[i])

        AUC_column,PR_column = draw_roc_column_test(model, columns, all_column_test_data, save_all_count_A[i], Sm, Sd, Lm, Md)
        CNN_AUC_all_column.extend(AUC_column)
        CNN_PR_all_column.extend(PR_column)

    print("CNN")
    AUC_test.append(CNN_AUC_all_column)
    PR_test.append(CNN_PR_all_column)
    SIMC_AUC_all_column, SIMC_PR_all_column = SIMC.test2_test(A, Sd, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    print("SIMC")
    AUC_test.append(SIMC_AUC_all_column)
    PR_test.append(SIMC_PR_all_column)
    LDAP_AUC_all_column, LDAP_PR_all_column = LDAP.test3_test(A, save_all_count_A, save_all_count_zero_not_changed,
                                    save_all_count_zero_every_time_changed, num, A_length, sava_association_A)
    print("LDAP_Rcall")
    AUC_test.append(LDAP_AUC_all_column)
    PR_test.append(LDAP_PR_all_column)
    ANML_AUC_all_column, ANML_PR_all_column = ANML.test4_test(A, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    print("ANML_Rcall")
    AUC_test.append(ANML_AUC_all_column)
    PR_test.append(ANML_PR_all_column)
    MFLDA_AUC_all_column, MFLDA_PR_all_column = MFLDA.test5_test(A, save_all_count_A, sava_association_A, save_all_count_zero_not_changed,
                                       save_all_count_zero_every_time_changed, num, A_length)
    print("MFLDA_Rcall")
    AUC_test.append(MFLDA_AUC_all_column)
    PR_test.append(MFLDA_PR_all_column)
...
def test8_test(k):
    ...
    for i in range(k):
        ...
        del model
        # time += 1
    print('CNNLDA_AUC',area_all_ROC / k)
    AUC_test.append(CNN_AUC_all_column)
    DL_FPR, DL_TPR, DL_R, DL_P = get_all_k_Curve(X_all_k_FPR, Y_all_k_TPR, X_all_k_R, Y_all_k_P, k)
    PR_test.append(CNN_PR_all_column)
    dis_15_AUC = np.sum(dis_15_AUC, axis=0) / len(dis_15_AUC)
    dis_15_PR = np.sum(dis_15_PR, axis=0) / len(dis_15_PR)
    dis_15_AUC_all.append(dis_15_AUC)
    dis_15_PR_all.append(dis_15_PR)
    # CNNLDA_15_AUC_PR = []
    # CNNLDA_15_AUC_PR.append(dis_15_AUC)
    # CNNLDA_15_AUC_PR.append(dis_15_PR)
    # np.savetxt('CNNLDA_15_AUC_PR.txt', CNNLDA_15_AUC_PR)

    # np.savetxt('CNNLDA_PR_test2.txt', CNN_PR_all_column)
    print('CNNLDA_PR',area_all_PR / k)
    print("CNN ok!")
    # save_to_file2('cnnlncRNA.txt', d)
    ...
    SIMC_FPR, SIMC_TPR,SIMC_R,SIMC_P,SIMC_AUC,SIMC_PR,SIMC_Rcall,SIMC_AUC_all_column, SIMC_PR_all_column,SIMC_15_AUC,SIMC_15_PR = SIMC.test2_test(A, Sd, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    AUC_test.append(SIMC_AUC_all_column)
    PR_test.append(SIMC_PR_all_column)
    SIMC_15_AUC = np.sum(SIMC_15_AUC, axis=0) / len(SIMC_15_AUC)
    SIMC_15_PR = np.sum(SIMC_15_PR, axis=0) / len(SIMC_15_PR)
    dis_15_AUC_all.append(SIMC_15_AUC)
    dis_15_PR_all.append(SIMC_15_PR)
    ...
    LDAP_FPR, LDAP_TPR,LDAP_R,LDAP_P,LDAP_AUC,LDAP_PR,LDAP_Rcall,LDAP_AUC_all_column, LDAP_PR_all_column,LDAP_15_AUC,LDAP_15_PR = LDAP.test2_test(A, save_all_count_A, save_all_count_zero_not_changed,
                                    save_all_count_zero_every_time_changed, num, A_length, sava_association_A)
    ...
    AUC_test.append(LDAP_AUC_all_column)
    PR_test.append(LDAP_PR_all_column)
    LDAP_15_AUC = np.sum(LDAP_15_AUC, axis=0) / len(LDAP_15_AUC)
    LDAP_15_PR = np.sum(LDAP_15_PR, axis=0) / len(LDAP_15_PR)
    dis_15_AUC_all.append(LDAP_15_AUC)
    dis_15_PR_all.append(LDAP_15_PR)

    ANML_FPR, ANML_TPR,ANML_R,ANML_P,ANML_AUC,ANML_PR,ANML_Rcall,ANML_AUC_all_column, ANML_PR_all_column,ANML_15_AUC,ANML_15_PR = ANML.test3_test(A, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)
    ...
    AUC_test.append(ANML_AUC_all_column)
    PR_test.append(ANML_PR_all_column)
    ANML_15_AUC = np.sum(ANML_15_AUC, axis=0) / len(ANML_15_AUC)
    ANML_15_PR = np.sum(ANML_15_PR, axis=0) / len(ANML_15_PR)
    dis_15_AUC_all.append(ANML_15_AUC)
    dis_15_PR_all.append(ANML_15_PR)

    MFLDA_FPR, MFLDA_TPR,MFLDA_R,MFLDA_P,MFLDAC_AUC,MFLDA_PR,MFLDA_Rcall,MFLDA_AUC_all_column, MFLDA_PR_all_column,MFLDA_15_AUC,MFLDA_15_PR = MFLDA.test2_test(A, save_all_count_A, sava_association_A, save_all_count_zero_not_changed,
                                       save_all_count_zero_every_time_changed, num, A_length)
    ...
    P.append(MFLDA_P)
    P.append(LDAP_P)
    # np.savetxt('ROC_FPR1.txt', ROC_FPR)
    # np.savetxt('ROC_TPR1.txt', ROC_TPR)
    # np.savetxt('R1.txt', R)
    # np.savetxt('P1.txt', P)
    ...
    AUC_test.append(MFLDA_AUC_all_column)
    PR_test.append(MFLDA_PR_all_column)
    MFLDA_15_AUC = np.sum(MFLDA_15_AUC, axis=0) / len(MFLDA_15_AUC)
    MFLDA_15_PR = np.sum(MFLDA_15_PR, axis=0) / len(MFLDA_15_PR)
    dis_15_AUC_all.append(MFLDA_15_AUC)
    dis_15_PR_all.append(MFLDA_15_PR)

    # np.savetxt('AUC_Ttest1.txt',AUC_test)
    # np.savetxt('PR_Ttest1.txt', PR_test)  # new_area_PR
    np.savetxt('AUC_15_dis2.txt',dis_15_AUC_all)
    np.savetxt('PR_15_dis_last2.txt',dis_15_PR_all)
