...
class Beta_score2(nn.Module):
    def __init__(self,input_size_lnc,input_size_A,input_size_lncmi,input_size_AT,input_size_dis,input_size_midis, output_size,batch_size):
        ...
        nn.init.xavier_normal(self.node1.weight)
...
def train5(model,EPOCH,train_loader,test_loader,LR):
...
def test4(k):
    ...
    all_k_development, save_all_count_A, save_all_count_zero_not_changed, save_all_count_zero_every_time_changed = data_partitioning(A,k)
    epoch1 = 80
    epoch2 = 10
    _LR = 0.0001
    batchsize = 50
...
def test5(A, Sd, Lm, Md,all_k_Sd_associated, all_k_test_data,all_k_development, save_all_count_A):
    k = 5
    # A, Sd, Lm, Md = read_data_flies()
    ...
    d = 'area_ROC:{},area_PR:{},LR:{},epoch1:{},epoch2:{}'.format(a_a_1,a_a_2,_LR,epoch1,epoch2)
    print(d)
    FPR, TPR = get_all_k_Curve(X_all_k_FPR, Y_all_k_TPR, X_all_k_R, Y_all_k_P, k)
    return FPR, TPR
    # save_to_file2('cnnlncRNA.txt', d)
...
def test6(k):
    ...
    SIMC_FPR, SIMC_TPR,SIMC_R,SIMC_P = SIMC.test2(A, Sd, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)

    LDAP_FPR, LDAP_TPR,LDAP_R,LDAP_P = LDAP.test2(A, save_all_count_A, save_all_count_zero_not_changed,
                                    save_all_count_zero_every_time_changed, num, A_length, sava_association_A)

    ANML_FPR, ANML_TPR,ANML_R,ANML_P = ANML.test3(A, save_all_count_A, sava_association_A, save_all_count_zero_every_time_changed,
                                    num, A_length)

    MFLDA_FPR, MFLDA_TPR,MFLDA_R,MFLDA_P = MFLDA.test2(A, save_all_count_A, sava_association_A, save_all_count_zero_not_changed,
    ...
    l1 = plt.legend([p1, p2, p3, p4, p5],
                    ["DLACNNLDA(0.9416)", "SIMCLDA(0.7464)", "ANMLDA(0.8723)", "MFLDA(0.6545)", "LDAP(0.8826)"],
                    loc='lower right')
