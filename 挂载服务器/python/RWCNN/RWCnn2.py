...
def test(k):
    ...
    for alpha in which_alpha.values():  # 明确指出遍历字典的值
        for count in which_maxcountrw.values():
            all_k_Sd_associated, all_k_count_positive, all_k_test_data, A, num, \
            A_length, sava_association_A, all_k_development, k_pt_features, Sm, Sd = data_partitioning(alpha, k, count)
                        ...
                        for w_decay in which_weight_decay.values():
                            for _scheduler in which_Scheduler.values():
                                for _LR in which_LR.values():
                                    ...
                                    d = 'Time:{}, AUC:{}, alpha:{}, maxCount:{}, BatchSize:{}, Epoch:{}, optim:{}, weightdecay:{}, Scheduler:{}, LR:{}'.format(
                                        time, current_k_area, alpha, count, batchsize, epoch, _optim, w_decay, _scheduler, _LR)
