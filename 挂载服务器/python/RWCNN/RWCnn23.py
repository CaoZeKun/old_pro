...
def data_partitioning(alpha, k, max_count):
    A, Sm, Sd, save_all_count_A, sava_association_A, save_all_count_zero_not_changed, \
    save_all_count_zero_every_time_changed, num, A_length, k_pt_features = rw.get_k_pt(alpha, k, max_count)
    ...
    for x in range(k):
        ...
        for i in range(length):
            temp_save.append(
                data_input(save_all_count_zero[i].value, save_all_count_zero[i].value_x,
...
class CNN(nn.Module):
    def __init__(self):
        ...
        # nn.init.xavier_normal(self.conv1[0].weight.data)
        print(torch.typename(self.conv1[0]))
        print(torch.typename(self.conv1[1]))
        print(torch.typename(self.conv1[2]))
        print(torch.typename(self.conv1[3]))
        print(torch.typename(self.conv1[4]))
        print(self.conv1[0])
        self.conv2 = nn.Sequential(
            nn.Conv2d(16, 32, 2, 1, 0),  # may 32 * 1 * 815
            nn.Dropout(0.5),
            nn.BatchNorm2d(32),
            nn.ReLU(),
    ...
    for epoch in range(Epoch):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            if ((step + 1) % length_train_loader == 0) and (epoch == Epoch - 1):
                ...
                save_to_file2('test.txt', str(context))
...
def draw_roc_column1(columns, column_test_data, pt_feature, BATCH_SIZE):
    ...
    for i in range(columns):
        ...
        for step, (x, y) in enumerate(test_loader):
            ...
            predict_value = torch.max(F.softmax(out, dim=1), 1)[
...
def get_current_AUC_ROC(cnn, test_data, columns, pt_feature, time):
    ...
    Y_TPR_all_columns, X_FPR_all_columns, area_all_column, average_column_area = draw_roc_column(cnn, columns,
                                                                                                 all_column_test_data,
                                                                                                 pt_feature)
...
def validation_test(alpha, k, max_count, batchsize, EPOCH, LR, dev_batchsize):
    ...
    for i in range(k):
        ...
        x_all_FPR, y_all_TPR, current_k_area = get_current_AUC_ROC(test_data, len(A[0]), k_pt_features[i],
                                                                   i)  # all column
        if (i != k - 1):
...
def test(k):
    ...
    which_alpha = {'0.1': 0.1, '0.2': 0.2, '0.3': 0.3, '0.4': 0.4, '0.5': 0.5, '0.6': 0.6, '0.7': 0.7, '0.8': 0.8,
                   '0.9': 0.9}
    ...
    for alpha in which_alpha.values():  # 明确指出遍历字典的值
        all_k_Sd_associated, all_k_count_positive, all_k_test_data, A, num, \
        A_length, sava_association_A, all_k_development, k_pt_features, Sm, Sd = data_partitioning(alpha, k, 10)
...
if __name__ == '__main__':
    ...
    # cross validation
    # validation_test(alpha, k, max_count,batchsize,EPOCH,LR,dev_batchsize=50)
